"use client";

import { useEffect, useState } from "react";
import { FEATURE_FLAGS, readFlag, writeFlag } from "@/lib/feature-flags";

export default function TogglePage() {
  const [values, setValues] = useState<Record<string, boolean>>({});

  // Read flags from localStorage on mount (client-only, avoids SSR mismatch)
  useEffect(() => {
    const current = Object.fromEntries(FEATURE_FLAGS.map((flag) => [flag.key, readFlag(flag.key)]));
    setValues(current);
  }, []);

  const toggle = (key: string, value: boolean) => {
    writeFlag(key, value);
    setValues((prev) => ({ ...prev, [key]: value }));
  };

  return (
    <div className="min-h-screen bg-white text-black p-6">
      <h1 className="text-2xl font-bold mb-1">Feature toggles</h1>
      <p className="text-sm text-gray-500 mb-6">
        Poor man&apos;s feature toggles, stored in localStorage.
      </p>
      <ul className="space-y-4 max-w-xl">
        {FEATURE_FLAGS.map((flag) => (
          <li key={flag.key} className="flex items-start gap-3">
            <input
              id={flag.key}
              type="checkbox"
              className="mt-1 h-5 w-5"
              checked={values[flag.key] ?? false}
              onChange={(e) => toggle(flag.key, e.target.checked)}
            />
            <label htmlFor={flag.key} className="cursor-pointer">
              <span className="block font-medium">{flag.label}</span>
              <span className="block text-sm text-gray-500">{flag.description}</span>
            </label>
          </li>
        ))}
      </ul>
    </div>
  );
}
